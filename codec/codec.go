...
func (d *Decoder) Seek(uid uint64, whence seekPos) []uint64 {
	//fmt.Printf("Seek called with uid:%v and seekPos:%v\n", uid, whence)
	if d.Pack == nil {
		return []uint64{}
	}
	d.blockIdx = 0
	...
	blocksFunc := func() searchFunc {
		...
		switch whence {
		case SeekStart:
			f = func(i int) bool { return pack.Blocks[i].Base >= (uid & msbBitMask) }
		case SeekCurrent:
			f = func(i int) bool { return pack.Blocks[i].Base > (uid & msbBitMask) }
		}
		return f
	...
	if idx < len(pack.Blocks) && pack.Blocks[idx].Base == (uid&msbBitMask) {
		d.blockIdx = idx
		//return d.UnpackBlock()
		d.UnpackBlock()
		// TODO: seek to right position of uids
	}
